...
class Newi18nTextCollector extends i18nTextCollector
{
    ...
    public function run($restrictToModules = null, $mergeWithExisting = false)
    {
        ...
        if ($entitiesByModule) {
            foreach ($entitiesByModule as $module => $entities) {
                $results[$module] = $this->getWriter()->write(
                    $entities,
                    $this->defaultLocale,
                    $this->baseSavePath . '/' . $module
                );
            }
        }
    ...
    public function collect($restrictToModules = null, $mergeWithExisting = true)
    {
            ...
            if ($module != 'themes') {
                ...
                if (count($themes)) {
                    foreach ($themes as $theme) {
                        if (is_dir($this->basePath . "/themes/" . $theme) && substr(
                            $theme,
                            0,
                            1
        ...
        foreach ($modules as $module) {
            ...
            // themes folder)
            $isValidModuleFolder = (is_dir("$this->basePath/$module")
                && (is_file("$this->basePath/$module/_config.php") || (is_dir("$this->basePath/$module/_config")))
                && (substr($module, 0, 1) != '.') || (substr($module, 0, 7) == 'themes/' && is_dir("$this->basePath/$module")));
            ...
            if (isset($entitiesByModule[$module])) {
                $entitiesByModule[$module] = array_merge_recursive(
